...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    var totalTime = 0 {
        willSet{
            timeLable.text = newValue.convertToTimeFormat()
        }
    }
    ...
    var intervals = 0 {
        willSet{
            let cell = timingCell()
            cell?.detailTextLabel?.text = newValue.convertToTimeFormat()
        }
    }
    ...
    func setUI(){
        ...
        resetButton .setBackgroundImage(UIColor.init(red: 0.08, green: 0.08, blue: 0.08, alpha: 1).createImage(), for: .disabled)
        
        resetButton.setTitle("复位", for: .selected);
        resetButton.setTitleColor(UIColor.white, for: .selected)
        resetButton.setBackgroundImage(UIColor.gray.createImage(), for: .selected)
        ...
        startButton.setBackgroundImage(UIColor.init(red: 20 / 255.0, green: 40 / 255.0, blue: 24 / 255.0, alpha: 1).createImage(), for: .normal)
        ...
        startButton.setBackgroundImage(UIColor.init(red: 74 / 255.0, green: 16 / 255.0, blue: 17 / 255.0, alpha: 1).createImage(), for: .selected)
